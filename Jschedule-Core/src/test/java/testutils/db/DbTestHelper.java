...
//		in.close();
		resetDB(conn, in, parser);
	}

	public void resetDB(Connection conn, InputStream queryFileStream,
			SqlParser parser) throws IOException, SQLException{
		if ( queryFileStream == null ) {
			throw new SQLException("NULL query stream : " + queryFileStream);
		}
		executor = new QueryExecutor(conn);
		executor.runQueries(queryFileStream, parser);
	}
