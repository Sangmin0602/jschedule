...
CREATE TABLE IF NOT EXISTS `scheduledb`.`Schedules` (
  `seq` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `Users_seq` INT UNSIGNED NOT NULL,
  `Places_seq` INT UNSIGNED NOT NULL,
  `start_time` TIMESTAMP NOT NULL DEFAULT current_timestamp,
  `Participants_seq` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`seq`, `Places_seq`, `Participants_seq`),
  INDEX `fk_Schedules_Users_idx` (`Users_seq` ASC),
  INDEX `fk_Schedules_Places1_idx` (`Places_seq` ASC),
  INDEX `fk_Schedules_Participants1_idx` (`Participants_seq` ASC),
  ...
  CONSTRAINT `fk_Schedules_Places1`
    FOREIGN KEY (`Places_seq`)
    REFERENCES `scheduledb`.`Places` (`seq`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Schedules_Participants1`
    FOREIGN KEY (`Participants_seq`)
...
CREATE TABLE IF NOT EXISTS `scheduledb`.`SchedulePatterns` (
  ...
  `period_mins` INT UNSIGNED NOT NULL DEFAULT 0,
  `period_counts` INT UNSIGNED NOT NULL DEFAULT 1,
  `adjust_mins` INT NOT NULL DEFAULT 0,
  PRIMARY KEY (`seq`, `fk_schedules`),
  ...
  CONSTRAINT `fk_SchedulePatterns_Schedules1`
    ...
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = '일정의 반복주기와 횟수를 기록하는 테이블. 하나의 일정이 여러개의 패턴을 가질 수 있음.';
...
-- -----------------------------------------------------
-- Table `scheduledb`.`UserEvents_has_Participants`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `scheduledb`.`UserEvents_has_Participants` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `scheduledb`.`UserEvents_has_Participants` (
  `seq` INT UNSIGNED NOT NULL AUTO_INCREMENT,
